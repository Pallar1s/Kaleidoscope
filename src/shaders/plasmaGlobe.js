export const plasmaGlobeFragmentShader = `
...
float noise(in vec3 p)
{
	...
	vec2 rg = texture(iChannel0, (tap + 0.5)/256.0).yx;
...
{   
    ...
    for( int i=0; i<VOLUMETRIC_STEPS; i++ )
    {
        ...
        sum += abs(col)*5. * (1.2-noise(lp*2.+j*13.+r.x*5.)*1.1) / (log(distance(p,orig)-2.)+.75);
    }
    return sum;
}
...
void main() {
    ...
    mat2 mx = mm2(u_time*.4);
    mat2 my = mm2(u_time*0.3); 
    ro.xz *= mx;rd.xz *= mx;
    ro.xy *= my;rd.xy *= my;
    ...
    for (float j = 1.;j<NUM_RAYS+1.;j++)
    {
        ro = bro;
        rd = brd;
        mat2 mm = mm2((u_time*0.1+((j+1.)*5.1))*j*0.25);
        ro.xy *= mm;rd.xy *= mm;
        ro.xz *= mm;rd.xz *= mm;
    ...
    if (sph.x > 0.)
    {
        ...
        float nz = (-log(abs(flow(rf*1.2,u_time)-.01)));
        float nz2 = (-log(abs(flow(rf2*1.2,-u_time)-.01)));
